(() => {
  ...
  // Auto-pick API based on where the page is running
  ...
  const CONFIG = {
    apiUrl: isLocalHost
      ...
      : "https://<YOUR_PROJECT_REF>.functions.supabase.co/chat", // prod function URL (replace!)
  ...
  // If other code needs CONFIG globally, uncomment:
  ...
  const storage = window.sessionStorage; // swap to localStorage if you want persistence across browser restarts
  ...
  function historyFromDOM(bodyEl) {
    return Array.from(bodyEl.querySelectorAll('.jw-msg')).map(el => ({
      role: el.classList.contains('jw-user') ? 'user' : 'assistant',
      content: el.textContent || ''
    }));
  }

  function saveHistoryFromDOM(bodyEl) {
    const trimmed = historyFromDOM(bodyEl).slice(-50);
    storage.setItem(STORAGE_KEY, JSON.stringify(trimmed));
  }

  // ====== Styles ======
  const styles = `
  .jw-chat-root { position: fixed; font-family: system-ui, -apple-system, Segoe UI, Roboto, Arial, sans-serif; z-index: 999999; }
  .jw-chat-button { border: none; cursor: pointer; box-shadow: 0 10px 20px rgba(0,0,0,0.15); width: 56px; height: 56px; display:flex; align-items:center; justify-content:center; color: #fff; }
  .jw-chat-panel { width: 360px; max-height: 70vh; display: none; flex-direction: column; overflow: hidden; background: #111827; color: #fff; box-shadow: 0 20px 50px rgba(0,0,0,0.35); }
  ...
  @media (max-width: 480px) { .jw-chat-panel { width: 92vw; max-height: 76vh; } }
  `;
  ...
  root.className = 'jw-chat-root';
  root.style.bottom = CONFIG.position.bottom + 'px';
  root.style.right = CONFIG.position.right + 'px';

  const btn = document.createElement('button');
  btn.className = 'jw-chat-button';
  btn.style.background = CONFIG.bubbleBg;
  btn.style.borderRadius = CONFIG.cornerRadius + 'px';
  btn.innerHTML = `
  ...
  panel.className = 'jw-chat-panel';
  panel.style.borderRadius = CONFIG.cornerRadius + 'px';
  ...
  panel.innerHTML = `
    <div class="jw-chat-header" style="background:${CONFIG.brandColor}">
      ...
      <div>
        <button class="jw-clear" title="Clear chat">↺</button>
  ...
  if (saved.length) {
    saved.forEach(m => pushMsg(m.content, m.role === 'user' ? 'user' : 'bot'));
    body.dataset.welcomed = '1';
  }

  function pushMsg(text, who) {
    const div = document.createElement('div');
    div.className = `jw-msg ${who === 'user' ? 'jw-user' : 'jw-bot'}`;
    div.textContent = text;
    body.appendChild(div);
    body.scrollTop = body.scrollHeight;

    // persist after each message
    saveHistoryFromDOM(body);
  }
  ...
  async function sendMsg() {
    ...
    try {
      // Build history only (no system prompt here; server owns persona)
      ...
      pushMsg(data.reply || 'Hmm, I did not catch that.', 'bot');
    } catch (err) {
      pushMsg('Sorry—something went wrong. Try again in a moment.', 'bot');
    } finally {
